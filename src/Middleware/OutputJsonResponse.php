...
namespace LaravelApiResponse\Middleware;
...
use LaravelApiResponse\Exceptions\ExceptionHandler;
use LaravelApiResponse\Exceptions\JsonResponseErrorException;
use LaravelApiResponse\Traits\HasJson;
        ...
        if ($_response && !$_response->exception) {
            if ($content = $_response->getOriginalContent()) {
                ...
                if ($content instanceof Model) {
                    $this->json()->set(Str::snake(last(explode("\\", get_class($content)))), $content);
